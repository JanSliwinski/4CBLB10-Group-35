...
% The calculation assumes standard conditions (T_in = 298.15 K).

% Function Signature:
function [deltaH_combustion] = ENTHALHeatOfCombustion(fuel_name, SpS, stoich_coeffs)
...
%    - Use the standard enthalpy of formation H_f for species at 298.15 K when available.
    ...
    if length(T_ranges) == 3
        ...
        T_low = T_ranges(1);
        T_common = T_ranges(1); %CHECK THIS!!! WHAT SIZE IS T RANGES??
        T_high = T_ranges(end);
    end
